...
Set ``TYPEMAP_BACKEND=numpy`` to force the numpy path.
...
def _mode() -> str:
    """TYPEMAP_BACKEND: '' / 'onnx' / 'cpu' (auto), 'numpy' (force numpy),
    'gpu' / 'cuda' (force CUDA, fail fast if it cannot initialise)."""
    return os.environ.get("TYPEMAP_BACKEND", "").strip().lower()
...
def _providers(model: str = "unet"):
    ...
    if _require_gpu():
        if "CUDAExecutionProvider" not in avail:
            raise RuntimeError(
                f"TYPEMAP_BACKEND={_mode()} requested but CUDAExecutionProvider is not "
                "available. Install the GPU backend: pip install \"typemap[gpu]\"."
            )
...
def available() -> bool:
    ...
    With ``TYPEMAP_BACKEND=gpu``/``cuda`` a missing onnxruntime or missing graphs is a
    ...
    try:
        ...
        if _require_gpu():
            raise RuntimeError(
                f"TYPEMAP_BACKEND={mode} requires the ONNX GPU backend, but onnxruntime "
                "is not installed. Install with: pip install \"typemap[gpu]\"."
            ) from exc
        return False
    try:
    ...
    if not ok and _require_gpu():
        raise RuntimeError(f"TYPEMAP_BACKEND={mode} requested but the bundled ONNX graphs are missing.")
    return ok
...
def _session(name: str):
    ...
    try:
        ...
        if _require_gpu():
            raise RuntimeError(
                f"TYPEMAP_BACKEND={_mode()}: the CUDA execution provider failed to "
    ...
    if _require_gpu() and "CUDAExecutionProvider" not in sess.get_providers():
        ...
        raise RuntimeError(
            f"TYPEMAP_BACKEND={_mode()}: CUDA was requested but the session loaded only "
