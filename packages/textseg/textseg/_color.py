...
``NO_COLOR`` and the ``TEXTSEG_COLOR`` (``auto``/``always``/``never``) override.
"""
...
def color_enabled(stream: Optional["object"] = None) -> bool:
    """Whether to emit ANSI colour. ``TEXTSEG_COLOR`` wins, then ``NO_COLOR``,
    else on only when ``stream`` (default stdout) is a TTY."""
    mode = os.environ.get("TEXTSEG_COLOR", "auto").lower()
