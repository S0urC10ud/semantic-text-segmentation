#!/usr/bin/env bash
# run_smaller_maba.sh — Start the GCP VM with a self-contained training job.
#
...
#
# Usage:  ./scripts/run_smaller_maba.sh
#
# To monitor:
#   gcloud compute ssh martindallinger2002_gmail_com@thesis-l4-2 \
...
echo "=============================="
echo "🏋️ Starting training from scratch ..."
echo "  \$(date)"
echo "=============================="

python "${REPO_DIR}/train/main.py" \
    --arch mamba --model_dim 256 --mamba_layers 6 --mamba_d_state 16 --mamba_expand 1 \
    --batch_size 32 --accum_steps 1 --lr 1.414e-4 --steps 150000 --eval_every 250 \
