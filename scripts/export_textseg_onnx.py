"""Build dynamic-length ONNX graphs for the textseg U-Net and Mamba models.

Build-time tool only (needs ``onnx``; NOT a runtime dependency of the package).
Reads the slimmed weights already bundled at ``packages/textseg/textseg/data``
...
remap before feeding the session. Math mirrors ``textseg._numpy_backend`` exactly.
...
DATA = Path(__file__).resolve().parents[1] / "packages" / "textseg" / "textseg" / "data"
OPSET = 17
...
def build_unet(w: dict, channels, num_classes: int, seq: int = 1536) -> onnx.ModelProto:
    ...
    graph = helper.make_graph(g.nodes, "textseg_unet", [inp], [out], g.inits)
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", OPSET)],
                              ir_version=9)
    model.doc_string = "textseg U-Net (fixed 1536 window, dynamic batch)"
...
def build_mamba(w: dict, n_layers: int, d_state: int, dt_rank: int,
    ...
    graph = helper.make_graph(g.nodes, "textseg_mamba", [inp], [out], g.inits)
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", OPSET)],
                              ir_version=9)
    model.doc_string = "textseg Mamba (dynamic sequence length via Scan)"
    onnx.checker.check_model(model)
    return model
